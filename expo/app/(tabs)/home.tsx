...
// Или если '@/components/Gradient' уже на этой lib, импортируй оттуда
import { SafeAreaView } from 'react-native-safe-area-context';
import { useUserStore } from '@/stores/user-store';
import {
  UserRound, ChevronRight, Globe, Timer, Hourglass, AlarmClock, BedDouble
} from 'lucide-react-native';
import { useNavigation } from '@react-navigation/native'; // Новый импорт для навигации
import Svg, { Circle, Path, Defs, LinearGradient as SvgGradient, Stop } from 'react-native-svg';
import { trigger } from 'react-native-haptic-feedback'; // Замена Haptics

const DIAL_STROKE = 16;
const TAP_MOVE_THRESHOLD_DEG = 1.25;
const TOUCH_BAND = DIAL_STROKE / 2 + 20;
const MIN_GAP_MIN = 1;
const STEP_MIN = 5;
const COLORS = {
...
const DIAL_SIZE = Math.min(360, Math.max(260, Math.round(Dimensions.get('window').width * 0.78)));
const HANDLE_ICON_BOX = 48;
const normMin = (m) => ((m % 1440) + 1440) % 1440;
const floor5 = (m) => Math.floor(m / STEP_MIN) * STEP_MIN;
const ceil5 = (m) => Math.ceil(m / STEP_MIN) * STEP_MIN;
...
export default function HomeScreen() {
  ...
  const [bed, setBedState] = useState(() => toMinutes(sleepPlan.bedTime));
  const [wake, setWakeState] = useState(() => toMinutes(sleepPlan.wakeTime));
  const bedRef = useRef(bed);
  const wakeRef = useRef(wake);
  const setBed = (v) => { bedRef.current = v; setBedState(v); };
  const setWake = (v) => { wakeRef.current = v; setWakeState(v); };
  ...
  const minGapMin = useMemo(() => {
    ...
    const visualPx = HANDLE_ICON_BOX + DIAL_STROKE + 6;
    const minutes = Math.ceil(visualPx / arcPerMin);
    return Math.max(MIN_GAP_MIN, minutes);
  }, [radius]);

  const minutesToAngle = useCallback((min) => (min / 1440) * 360 - 90, []);
  const pointOnCircle = useCallback((ang) => {
    const rad = (ang * Math.PI) / 180;
    return { x: center.x + radius * Math.cos(rad), y: center.y + radius * Math.sin(rad) };
  }, [center.x, center.y, radius]);

  const degDist = (a, b) => Math.abs(((a - b + 180) % 360) - 180);
  const angleFromTouch = useCallback((evt) => {
  ...
  const isInRing = useCallback((evt) => {
    const { locationX, locationY } = evt.nativeEvent;
    const dx = locationX - center.x, dy = locationY - center.y;
  ...
  const startAngRef = useRef(0);
  const lastShownMinRef = useRef(-1);
  const prevAngRef = useRef(0);
  const dragValueRef = useRef(0);

  const clampWithBarrier = useCallback((which, raw) => {
  ...
  const snapAndProtect = useCallback((which, val) => {
  ...
  const panResponder = useRef(
    PanResponder.create({
      onStartShouldSetPanResponder: (evt) => isInRing(evt),
      ...
      onPanResponderGrant: (evt) => {
        ...
        if (Platform.OS === 'ios') {
          try { trigger('selectionStart'); } catch {} // Замена Haptics.selection()
        }
      ...
      onPanResponderMove: (evt) => {
        ...
        const delta = ((deltaRaw + 540) % 360) - 180;
        prevAngRef.current = ang;
        ...
        if (showMin !== lastShownMinRef.current && Platform.OS !== 'web') {
          ...
          trigger('selectionStart'); // Замена Haptics.selection()
        }
      },
        ...
        <LinearGradient colors={COLORS.grad} style={{ flex: 1 }}>
          <SafeAreaView style={styles.safeArea}>
            <View style={styles.header} pointerEvents="box-none">
              ...
              <TouchableOpacity onPress={() => navigation.navigate('settings')} activeOpacity={0.9}>
            ...
            <View style={styles.contentWrap}>
              <View style={styles.dialWrap}>
                <View style={styles.svgHolder} {...panResponder.panHandlers} testID="sleep-dial">
                  ...
                  <View style={styles.centerTapArea} pointerEvents={isDragging ? 'none' : 'box-none'}>
                    <TouchableOpacity disabled={isDragging} activeOpacity={0.9} onPress={() => navigation.navigate('sleepSettings')} style={styles.centerPress}>
              ...
              <TouchableOpacity style={styles.scheduleCard} onPress={() => navigation.navigate('sleepSettings')} activeOpacity={0.85}>
              ...
              <View style={styles.actionsRow}>
                <ActionButton label={t.stopwatch} icon={<Timer size={22} color={COLORS.text} />} onPress={() => navigation.navigate('settings')} />
                <ActionButton label={t.timer} icon={<Hourglass size={22} color={COLORS.text} />} onPress={() => navigation.navigate('settings')} />
                <ActionButton label={t.worldTime} icon={<Globe size={22} color={COLORS.text} />} onPress={() => navigation.navigate('settings')} />
              </View>
              <TouchableOpacity style={[styles.cta, { marginBottom: CARD_SPACING }]} activeOpacity={0.9} onPress={() => navigation.navigate('assistant')} testID="voice-cta">
